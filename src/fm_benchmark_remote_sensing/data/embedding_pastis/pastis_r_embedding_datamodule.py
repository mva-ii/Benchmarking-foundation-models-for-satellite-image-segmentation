...
from fm_benchmark_remote_sensing.data.embedding_pastis.fm_alphaearth import AlphaEarthFM
from fm_benchmark_remote_sensing.data.embedding_pastis.fm_tessera import TesseraFM
...
def build_fm(name: str, embedding_root: Path) -> FMBase:
    ...
    if key == "tessera":
        return TesseraFM(embedding_root)
    if key == "alphaearth":
        return AlphaEarthFM(embedding_root)
    if key == "alise":
        return AliseFM(embedding_root)
...
class EmbeddingDataModule(L.LightningDataModule):
    ...
    def __init__(
        self,
        pastis_r_root: Path,
        embedding_type: Literal["tessera"] | Literal["alphaearth"] | Literal["alise"],
        embedding_root: Path,
        batch_size: int,
        num_workers: int,
        pin_memory: bool,
        ...
        self.fm = build_fm(embedding_type, embedding_root)
